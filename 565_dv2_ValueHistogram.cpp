...
vector <string> ValueHistogram::build(vector <int> values) {
	...
	for(int i=0; i<values.size();i++) {
		int temp  = ++mValues[values[i]];
		maxRow = max(maxRow, temp+1);
	}

	vector<string> result(maxRow,row);
	
	for(map<int, int>::iterator mItem = mValues.begin(); mItem!= mValues.end(); mItem++) {
		for(int i = 0; i<mItem->second; i++) {
			result[maxRow -1-i][mItem->first] = 'X';
		}
	}
